#Exceeds requirements, I work the full requirements including the sum_protons function

from formula import parse_formula

def make_periodic_table():
    periodic_table_dict = {
        # [symbol, name, atomic_mass, number_of_protons]
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict, total_molar_mass = 0):
    ...
    for inner_list in symbol_quantity_list:
        ...
        if symbol in periodic_table_dict:
            ...
            #s_name = element[NAME_INDEX]
            a_mass = element[ATOMIC_MASS_INDEX]
        # Multiply the atomic mass by the quantity.
        product = quantity * a_mass
...
def sum_protons(symbol_quantity_list, periodic_table_dict, total_protons = 0):
    ...
    for inner_list in symbol_quantity_list:
        ...
        if symbol in periodic_table_dict:
            ...
            number_of_atoms = element[NUMBER_OF_ATOMS]

        total_protons += number_of_atoms
...
def main(): 
    ...
    formula = str(input("Please type the chemical formula for a molecule : "))
    # Get the mass of a chemical sample in grams from the user.
    mass_gr = float(input("Please type the mass of a chemical sample in grams: "))
    ...
    total = compute_molar_mass(symbol_quantity_list,table)
    # Compute the number of moles in the sample.
    moles = mass_gr / total
    # Print the molar mass.
    print(f"{total:.5} grams/mole")
    # Print the number of moles.
    print(f"{moles:.5} moles")

    compound_name = get_formula_name(formula, known_molecules_dict)
    total_protons = sum_protons(symbol_quantity_list, table)

    print(f"The fomula name is: {compound_name}")
    print(f"The number of protons is: {total_protons}")
